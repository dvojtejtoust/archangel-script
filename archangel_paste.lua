...
local info = {
    name = _USER_NAME and _USER_NAME or 'Admin',
    build = _SCRIPT_NAME and _SCRIPT_NAME or "Archangel Secret",
}

local antiaim_cond = {'Global', 'Standing', 'Walking', 'Running', 'Air', 'Air crouch', 'Crouch', 'Crouch+', 'Freestanding', 'Manuals'}
local lines21 = aa_gp:label('\vConfigurations Settings')
local list_select = aa_gp:multiselect("\ndwa", {"Information", "Ragebot", "Anti-Aim", "Visuals", "Miscellaneous"})

local function make_mock_element(name, defaults)
    defaults = defaults or {}
    local element = setmetatable({}, {
        __index = function(self, key)
            if key == "ref" then
                return menu and menu.main and menu.main.ref or 0
            end
            local index = {
                get = function(self, ...)
                    local arg = ...
                    if arg ~= nil then
                        if defaults.multiselect then
                            return false
                        end
                        return nil
                    end
                    ...
                    return defaults.value or false
                end,
                set = function(self, ...) return self end,
                set_callback = function(self, ...) return self end,
                depend = function(self, ...) return self end,
                set_visible = function(self, ...) return self end,
                set_enabled = function(self, ...) return self end,
            }
            return index[key]
        end
    })
...
local menu = {
    ...
    main = fl_gp:combobox("\nadawdwada", {"Home", " Information", " Ragebot", " Anti-Aim", " Visuals", " Miscellaneous"}),
    btn_rage = fl_gp:button("Ragebot System"),
    btn_aa = fl_gp:button("Anti-aim Tehnology"),
    btn_vis = fl_gp:button("Visualizations"),
    btn_misc = fl_gp:button("Miscellaneous"),
    btn_cfg = fl_gp:button("Config System"),
    btn_back = fl_gp:button("Back"),

    information = {
        user = aa_gp:label("\vUser: \r" .. info.name),
        build = aa_gp:label("\vBuild: \r" .. info.build:gsub("Archangel", " ")),
        linessoc = aa_gp:label(" "),
        session = aa_gp:label("\vSession: "),
        time = aa_gp:label('Time: '),
        kills = aa_gp:label('Kills: '),
        deaths = aa_gp:label('Deaths: '),
    ...
    ragebot = {
        lb_ = fl_gp:label(" "),
        ai = aa_gp:checkbox("AI Peek Settings"),
        ...
        ai_weapon = aa_gp:multiselect("Weapons Selection", {"Pistol", "Deagle", "Revolver", "Scout", "Auto", "Awp"}),
    ...
    antiaim = {  
        ...
        high_distance_options = aa_gp:multiselect('\nadwad', {'Standing', 'Running', 'Walking', 'Air', 'Air crouch', 'Crouch', 'Crouch+', 'Manuals', 'Freestanding'}),
    ...
    visuals = {
        category = aa_gp:combobox("\nCategory", {"\aC8A2C8FF \aFFFFFFFFGeneral", "\aC8A2C8FF \aFFFFFFFFIndicators", "\aC8A2C8FF \aFFFFFFFFCrosshair & Damage", "\aC8A2C8FF \aFFFFFFFFSolus UI", "\aC8A2C8FF \aFFFFFFFFOther"}),
        ...
        tracer_en = fl_gp:checkbox('\nawdwada'),
        tracer_lb = fl_gp:label('\vTracer'),
        
        lb_world_to_screen = fl_gp:label('World Screen'),
        kibit_marker = fl_gp:multiselect("\nadwad", {'Hit', 'Miss'}),
        kibit_color = fl_gp:color_picker(" ", 83, 255, 248),
        kibit_color2 = fl_gp:color_picker(" ", 255, 0, 0),
        kibit_miss = fl_gp:checkbox("Miss reason"),

        --feature_ind
        custom_ind_gs_label = fl_gp:label('\vFeature indicators'),
        custom_ind_gs = fl_gp:checkbox('Enable animations\nfeature'),
        custom_ind_gs_color = fl_gp:checkbox('Enable custom color'),
        custom_ind_gs_colorcp = fl_gp:color_picker('cgsc', 255, 255, 255),
        custom_ind_gs_icon = fl_gp:checkbox('Add feature icons'),
        custom_ind_gs_size = fl_gp:slider('Adding to y', 6, 12, 12),
        custom_ind_gs_rect1 = fl_gp:slider('Weight size', 0, 15, 0),
        custom_ind_gs_rect = fl_gp:slider('Height size', 0, 4, 0),
        custom_ind_gs_back = fl_gp:slider('Background alpha', 25, 155, 50),

        --arrows
        arrows = fl_gp:checkbox('\nEnablearrows'),
        arrows_label = fl_gp:label('\vArrows'),
        arrows_color = fl_gp:color_picker('ac', 255, 255, 255),
        arrows_type = fl_gp:combobox('\nadwadw', {'Archangel', 'V4'}),
        v4_mode = fl_gp:multiselect('\nV4 Mode', {'Arrows', 'Desync'}),
        arrows_scope = fl_gp:multiselect('On scope', {'Alpha', 'Level'}),

        --crosshair
        crosshair = fl_gp:checkbox('\nEnablecri'),
        crosshair_label = fl_gp:label('\vCrosshair indicator'),
        cr_color_type = fl_gp:combobox("\nColor Type", {"Default", "Gradient"}),
        cr_color_gradient = fl_gp:combobox("\nColor Gradient", {"2x", "3x"}),
        cr_color_speed = fl_gp:slider("\nColor Speed", 1, 5, 2, true, " ", 1),
        cr_lb_default_color = fl_gp:label("\vDefault Color"),
        cr_default_color = fl_gp:color_picker("\nColor", 255, 255, 255),
        cr_lb_color1 = fl_gp:label("\vFirst Color"),
        cr_color1 = fl_gp:color_picker("\nColor", 255, 255, 255),
        cr_lb_color2 = fl_gp:label("\vSecond Color"),
        cr_color2 = fl_gp:color_picker("\nColor", 255, 255, 255),
        cr_lb_color3 = fl_gp:label("\vThird Color"),
        cr_color3 = fl_gp:color_picker("\nColor", 255, 255, 255),
        crosshair_select = fl_gp:multiselect('Statements', {'Glow module', 'Desync', 'State', 'Double tap', 'Onshot anti-aim', 'Body aim', 'Safe point', 'Duck peek assist', 'Minimum damage override', 'Freestanding'}),

        --damage
        damage_ind = fl_gp:checkbox('\nEnabledamageind'),
        damage_label = fl_gp:label('\vDamage indicator'),
        damage_color = fl_gp:color_picker('dc', 255, 255, 255),
        damage_anim = fl_gp:checkbox('Disable animation\ndamageind'),
        damage_select = fl_gp:combobox('Font', {'Small', 'Verdana', 'Bold'}),
        ...
        lb_hotkeys = oth_group:label("\vHotkeys"),
        ...
        ui_lb_color4 = oth_group:label("\vFourth Color"),
        ui_color4 = oth_group:color_picker("\nColor", 255, 255, 255),
        ui_lb_color5 = oth_group:label("\vFifth Color"),
        ui_color5 = oth_group:color_picker("\nColor", 255, 255, 255),
        aspect_ratio = fl_gp:checkbox('Force aspect ratio'),
        aspect_ratio_slider = fl_gp:slider('\nAmount', 60, 250, 178, true, ' ', 0.01, {[125] = '5:4', [133] = '4:3', [150] = '3:2', [160] = '16:10', [178] = '16:9', [200] = '2:1'}),
        third_person = fl_gp:checkbox('Third person'),
        third_person_slider = fl_gp:slider('\nDistance', 0, 250, 100),
        third_person_magic = fl_gp:checkbox('Disable third person (wall threshold)'),
        zoom = fl_gp:checkbox('Second zoom FOV'),
        zoom_hotkey = fl_gp:hotkey('\nZoom_ht', true),
        zoom_fov = fl_gp:slider("\nadwad", 5, 100, 10, true, "%", 1),
        zoom_anim = fl_gp:checkbox("Animate zoom FOV"),
        viewmodel = fl_gp:checkbox('Viewmodel'),
        opposite_hand = fl_gp:checkbox('Opposite knife hand'),
        remove_sleeves = fl_gp:checkbox('Remove Sleeves'),
        vS = fl_gp:slider('\nViewmodel FOV', 0, 170, 68, true, ' ', 1),
        xS = fl_gp:slider('\nViewmodel X', -250, 250, 0, true, ' ', 0.1),
        yS = fl_gp:slider('\nViewmodel Y', -250, 250, 0, true, ' ', 0.1),
        zS = fl_gp:slider('\nViewmodel Z', -250, 250, 0, true, ' ', 0.1),
        viewinsc = fl_gp:checkbox('Force weapon in scope'),
        scope_overlay = fl_gp:checkbox('Scope Overlay'),
        scope_mode_ovr = fl_gp:combobox("\nadwa", {"V1", "V2"}),
        scope_disablers = fl_gp:multiselect('Disabler', {'Left', 'Right', 'Bottom', 'Top'}),
        scope_color = fl_gp:color_picker('so', 255, 255, 255),
        scope_size = fl_gp:slider('Size', 0, 500, 190),
        scope_gap = fl_gp:slider('Gap', 0, 500, 15),
        scope_thickness = fl_gp:slider('Thickness', 1, 20, 1, true, " ", 1, {[1] = "Default"}),
        scope_position = fl_gp:combobox('Position', {'Default', 'Rotate'}),
        scope_animation = fl_gp:multiselect('Animations\nscoverl', {'Alpha', 'Overlay'}),
    },
    ...
    misc = {
        ...
        lb3 = aa_gp:label(" "),
        lb_aimbot = fl_gp:label("\vAimbot Logs"),
        output_log = fl_gp:combobox("\nOutput", {"Output", "Screen"}),
        aimbot_on = fl_gp:multiselect('\nLog when', {'Hit', 'Miss', 'Another'}),
        aimbot_on1 = fl_gp:multiselect('\nLog when', {'Hit', 'Miss', 'Another'}),
        aimbot_hit = fl_gp:color_picker('hit', 255, 255, 255),
        aimbot_miss = fl_gp:color_picker('miss', 255, 255, 255),
        aimbot_another = fl_gp:color_picker('another', 255, 255, 255),
        prefix_screen = fl_gp:checkbox('Prefix'),
        output_output = fl_gp:checkbox('Output'),
        output_lb = fl_gp:label(' '),
        output_font = fl_gp:combobox('Font', {'Small', 'Verdana', 'Bold'}),
        output_anim = fl_gp:checkbox('Enable animations'),
...
local function update_nav_visibility()
    local is_home = menu.main:get() == "Home"
    menu.btn_rage:set_visible(is_home)
    menu.btn_aa:set_visible(is_home)
    menu.btn_vis:set_visible(is_home)
    menu.btn_misc:set_visible(is_home)
    menu.btn_cfg:set_visible(is_home)
    menu.btn_back:set_visible(not is_home)
end

menu.main:set_callback(update_nav_visibility)

menu.btn_rage:set_callback(function() menu.main:set(" Ragebot"); update_nav_visibility() end)
menu.btn_aa:set_callback(function() menu.main:set(" Anti-Aim"); update_nav_visibility() end)
menu.btn_vis:set_callback(function() menu.main:set(" Visuals"); update_nav_visibility() end)
menu.btn_misc:set_callback(function() menu.main:set(" Miscellaneous"); update_nav_visibility() end)
menu.btn_cfg:set_callback(function() menu.main:set(" Information"); update_nav_visibility() end)

menu.btn_back:set_callback(function() menu.main:set("Home"); update_nav_visibility() end)

update_nav_visibility()

setmetatable(menu.ragebot, {
    __index = function(t, key)
        local dummy = setmetatable({}, {
            __index = function(self, k)
                ...
                local index = {
                    get = function(self, ...)
                        if key == "autostop" then
                            return "Off"
                        elseif key == "inter" then
                            return -1
                        elseif key == "esp_color" then
                            return 0, 0, 0, 0
                        elseif key == "esp_font" then
                            return "Off"
                        elseif key == "noscope_hit" or key == "noscope_dist" or key == "hitchance_force_shot" or key == "auto_tp_delay" then
                            return 0
                        elseif key == "cond" or key == "weapon" or key == "auto_tp_opt" or key == "auto_tp_wp" or key == "magic_hitbox" or key == "noscope_wp" or key == "esp_flags" or key == "force_shot_weapon" or key == "force_shot_weapons" or key == "hitrate" then
                            local arg = ...
                            if arg ~= nil then
                                return false
                            end
...
for i = 1, #antiaim_cond do
    ...
    builder[i] = {
        ...
        xway_mode = aa_gp:combobox("\nX-Way Mode", {"Automatic", "Manual"}),
        ...
        label11 = fl_gp:label('\vOther Settings'),
        delay_method = fl_gp:combobox("Delay Method", {"Default", "Ways"}),
        delay_switch = fl_gp:multiselect('\nawdad', {'Disable on Fakelags', 'Fluctuate', 'Hold ticks'}),
        delay = fl_gp:slider("Delay", 1, 16, 1, true, "t", 1, {[1] = " "}),
        random_delay = fl_gp:slider("Random Delay", 0, 22, 0, true, "t", 1, {[0] = " "}),
        delay_fluc = fl_gp:slider('Fluctuate', 1, 12, 1, true, '', 1, {[1] = ' '}),
        hold_ticks = fl_gp:slider("Hold ticks", 0, 14, 0, true, 't', 1, {[0] = ' '}),
        ways = fl_gp:slider("Ways", 3, 10, 3, true, "w", 1),
        way_del = {            
            [1] = fl_gp:slider('\nFirst offset \n', 1, 16, 1, true, "t", 1, {[1] = " "}),
            [2] = fl_gp:slider('\nSecond offset \n', 1, 16, 1, true, "t", 1, {[1] = " "}),
            [3] = fl_gp:slider('\nThird offset \n', 1, 16, 1, true, "t", 1, {[1] = " "}),
        ...
        label12 = fl_gp:label('\vDefensive Settings'),
        def_en = fl_gp:checkbox("Defensive"),
        def_activation = fl_gp:combobox("\nDefensive Activation", {"Always On", "On Peek"}),
        pitch_type = fl_gp:combobox('Pitch', {'Off', 'Static', 'Jitter', 'Spin', 'Spin[MOD]','Random', 'Random Ticks'}),
        pitch_static = fl_gp:slider('Angle', -89, 89, 0, true, '°', 1),
        pitch_mode1 = fl_gp:slider('Angle 1', -89, 89, 0, true, '°', 1),
        pitch_mode2 = fl_gp:slider('Angle 2', -89, 89, 0, true, '°', 1),
        pitch_speed = fl_gp:slider('Angle Speed', -50, 50, 20, true, ' ', 0.1),
        pitch_jitter_speed = fl_gp:slider("Speed Ticks", 2, 14, 2, true, 't', 1, {[2] = ' '}),
        pitch_slow_def = fl_gp:slider('Speed', 0, 10, 0, true, ' ', 0.1),
        yaw_def = fl_gp:combobox('Yaw', {'Off', 'Static', 'Static[FS]', 'Jitter', 'Jitter[L/R]', 'Spin', 'Random', '3-way', '5-way'}),
        yaw_amount = fl_gp:slider("Yaw amount", 0, 360, 0, true, '°', 1),
        yaw_left = fl_gp:slider('Yaw left', -180, 180, 0, true, '°', 1),
        yaw_right = fl_gp:slider('Yaw right', -180, 180, 0, true, '°', 1),
        def_jitter_speed = fl_gp:slider('Speed Ticks', 2, 14, 2, true, 't', 1, {[2] = ' '}),
        def_spin_speed = fl_gp:slider("Yaw Speed", -50, 50, 20, true, ' ', 0.1),
        adaptive_desync = fl_gp:multiselect("Yaw options", {'Adaptive Desync', 'Delay Off'}),
    }
end 
...
local aa_side = {} do
    ...
    function aa_side.save_to_custom(target_side)
        ...
        for i = 1, #antiaim_cond do
            ...
            v.xway_mode = b.xway_mode:get()
            v.xway_slider = b.xway_slider:get()
            v.xway_jitter = b.xway_jitter:get()
            v.xway_manual = v.xway_manual or {}
            for w = 1, 10 do
                v.xway_manual[w] = b.xway_manual[w]:get()
            end
            ...
            v.def_activation = b.def_activation:get()
    ...
    function aa_side.save()
        ...
        for i = 1, #antiaim_cond do
            ...
            builder_values[current_side][i].xway_mode = builder[i].xway_mode:get()
            ...
            builder_values[current_side][i].xway_manual = builder_values[current_side][i].xway_manual or {}
            for w = 1, 10 do
                builder_values[current_side][i].xway_manual[w] = builder[i].xway_manual[w]:get()
            end
            ...
            builder_values[current_side][i].def_activation = builder[i].def_activation:get()
    ...
    function aa_side.load()
        ...
        for i = 1, #antiaim_cond do
            ...
            pcall(function() 
                ...
                b.xway_mode:set(v.xway_mode or "Automatic")
                b.xway_slider:set(v.xway_slider or 3)
                b.xway_jitter:set(v.xway_jitter or 0)
                if v.xway_manual then
                    for w = 1, 10 do
                        b.xway_manual[w]:set(v.xway_manual[w] or 0)
                    end
                end
                ...
                b.def_activation:set(v.def_activation or "Always On")
...
local depend = {} do 
    ...
    function depend.builder(tab, tab2, current_side)
        for i = 1, #antiaim_cond do
            ...
            builder[i].xway_mode:depend(visible, state, cond, yaw, jitter, {builder[i].yaw_jitter, "X-Way"}, vis)
            builder[i].xway_slider:depend(visible, state, cond, yaw, jitter, {builder[i].yaw_jitter, "X-Way"}, vis)
            builder[i].xway_jitter:depend(visible, state, cond, yaw, jitter, {builder[i].yaw_jitter, "X-Way"}, {builder[i].xway_mode, "Automatic"}, vis)
            for w = 1, 10 do
                builder[i].xway_manual[w]:depend(visible, state, cond, yaw, jitter, {builder[i].yaw_jitter, "X-Way"}, {builder[i].xway_mode, "Manual"}, vis,
                    {builder[i].xway_slider, function()
                        if not builder[i].xway_slider then return false end
                        return builder[i].xway_slider:get() >= w
                    end}
                )
            end
            ...
            builder[i].label12:depend(visible, state, cond, def, vis)
            builder[i].def_en:depend(visible, state, cond, def, vis)
            builder[i].def_activation:depend(visible, state, cond, vis, def, {builder[i].def_en, true})
    ...
    function depend.visuals(tab)
        ...
        menu.visuals.category:depend(tab)
        
        -- General
        menu.visuals.globalize_dpi:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.dpilb:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.lb_recent:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.recent_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        
        menu.visuals.tracer_en:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.tracer_lb:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        
        menu.visuals.lb_world_to_screen:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.kibit_marker:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'})
        menu.visuals.kibit_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'}, {menu.visuals.kibit_marker, 'Hit'})
        menu.visuals.kibit_color2:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'}, {menu.visuals.kibit_marker, 'Miss'})
        menu.visuals.kibit_miss:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFGeneral'}, {menu.visuals.kibit_marker, 'Miss'})

        -- Indicators
        menu.visuals.custom_ind_gs_label:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_colorcp:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_icon:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_size:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_rect1:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_rect:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.custom_ind_gs_back:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})

        menu.visuals.arrows_label:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.arrows:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'})
        menu.visuals.arrows_type:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'}, {menu.visuals.arrows, true})
        menu.visuals.v4_mode:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'}, {menu.visuals.arrows, true}, {menu.visuals.arrows_type, "V4"})
        menu.visuals.arrows_scope:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'}, {menu.visuals.arrows, true})
        menu.visuals.arrows_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFIndicators'}, {menu.visuals.arrows, true})

        -- Crosshair & Damage
        menu.visuals.crosshair_label:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'})
        menu.visuals.crosshair:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'})
        menu.visuals.crosshair_select:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color_type:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color_gradient:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color_speed:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_lb_default_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Default"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_default_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Default"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_lb_color1:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "2x", "3x"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color1:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "2x", "3x"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_lb_color2:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "2x", "3x"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color2:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "2x", "3x"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_lb_color3:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "3x"}, {menu.visuals.crosshair, true})
        menu.visuals.cr_color3:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.cr_color_type, "Gradient"}, {menu.visuals.cr_color_gradient, "3x"}, {menu.visuals.crosshair, true})

        menu.visuals.damage_label:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'})
        menu.visuals.damage_ind:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'})
        menu.visuals.damage_anim:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.damage_ind, true})
        menu.visuals.damage_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.damage_ind, true})
        menu.visuals.damage_select:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFCrosshair & Damage'}, {menu.visuals.damage_ind, true})

        -- Solus UI
        menu.visuals.lb_ui:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'})
        menu.visuals.ui_main:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'})
        menu.visuals.ui_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark", "Keybinds", "Spectator", "Velocity", "Defensive"})
        menu.visuals.ui_lb_default_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark", "Keybinds", "Spectator", "Velocity", "Defensive"}, {menu.visuals.ui_color, "Once-color"})
        menu.visuals.ui_default_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark", "Keybinds", "Spectator", "Velocity", "Defensive"}, {menu.visuals.ui_color, "Once-color"})
        menu.visuals.ui_lb_color1:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_color1:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark"}, {menu.visuals.ui_color, "Multi-color"} )
        menu.visuals.ui_lb_color2:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Keybinds"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_color2:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Keybinds"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_lb_color3:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Spectator"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_color3:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Spectator"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_lb_color4:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Velocity"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_color4:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Velocity"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_lb_color5:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Defensive"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.ui_color5:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Defensive"}, {menu.visuals.ui_color, "Multi-color"})
        menu.visuals.lb_water:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark"})
        menu.visuals.water_main:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Watermark"})
        menu.visuals.hotkey_item:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Keybinds"})
        menu.visuals.lb_hotkeys:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFSolus UI'}, {menu.visuals.ui_main, "Keybinds"})
        
        -- Other
        menu.visuals.aspect_ratio:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.aspect_ratio_slider:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.aspect_ratio, true})
        menu.visuals.third_person:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.third_person_slider:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.third_person, true})
        menu.visuals.third_person_magic:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.third_person, true})
        menu.visuals.zoom:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.zoom_hotkey:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.zoom, true})
        menu.visuals.zoom_fov:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.zoom, true})
        menu.visuals.zoom_anim:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.zoom, true})
        menu.visuals.viewmodel:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.opposite_hand:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.remove_sleeves:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.vS:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.xS:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.yS:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.zS:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.viewmodel, true})
        menu.visuals.viewinsc:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.scope_overlay:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'})
        menu.visuals.scope_mode_ovr:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_disablers:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_color:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_size:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_gap:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_thickness:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_position:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
        menu.visuals.scope_animation:depend(tab, {menu.visuals.category, '\aC8A2C8FF \aFFFFFFFFOther'}, {menu.visuals.scope_overlay, true})
    end
    ...
    function depend.miscellaneous(tab, tab2, tab3)
        ...
        menu.misc.lb3:depend(tab, tab2)        menu.misc.lb_aimbot:depend(tab, tab2)
...
local antibrute = {} do
    ...
    function antibrute.trigger (event)
		...
		if (dists.mine and closest) and dists.mine < 40 or (closest == dists.mine and dists.mine < 128) then
            ...
            client.delay_call(0.01, function()
                ...
                client.fire_event("pizdec:Archangel:enemy_shot", {
                    damaged = false,
                    dist = dists.mine,
                    attacker = attacker,
                    userid = event.userid
    ...
    client.set_event_callback("pizdec:Archangel:enemy_shot", function(e)
        antibrute.schedule_brute(id)
    end)
...
local antiaim = {} do
    ...
    function antiaim.direction()
        ...
        if fs_active then
            ...
            (fs_disablers:get("Air") and jumpcheck) or 
        ...
        (m_disablers:get("Air") and jumpcheck) then
            yaw_direction = 0
        end
    ...
    function antiaim.xway(cmd)
        ...
        if ctx.yaw_jitter == "X-Way" then
            ...
            if ctx.xway_mode == "Manual" then
                local manual_val = ctx.xway_manual and ctx.xway_manual[way_index] or 0
                return manual_val
            end
    ...
    function antiaim.setup(cmd)
        ...
        elseif antiaim.state() == "Crouch" and get_cfg(7, real_side).enabled then
            id = 7
        elseif antiaim.state() == "Air crouch" and get_cfg(6, real_side).enabled then
            id = 6
        elseif antiaim.state() == "Air" and get_cfg(5, real_side).enabled then
            id = 5
        ...
        local is_peeking_active = ref.autopeek[1] and ref.autopeek[1]:get() or false
        local defensive_allowed = (cfg.def_activation == nil or cfg.def_activation == "Always On") or (cfg.def_activation == "On Peek" and is_peeking_active)

        if defensive and yaw_direction == 0 then
            if cfg.def_en and defensive_allowed and not menu.antiaim.fl_hot:get() then
                if cfg.def_en and defensive_allowed then
        ...
        if defensive and cfg.def_en and defensive_allowed and yaw_direction == 0 then
        ...
        if to_defensive and cfg.def_en and defensive_allowed then
        ...
        local high_state = {
            ...
            ["Crouch"] = menu.antiaim.high_distance_options:get("Crouch"),
            ["Air crouch"] = menu.antiaim.high_distance_options:get("Air crouch"),
            ["Air"] = menu.antiaim.high_distance_options:get("Air"),
...
local autostop = {} do
    ...
    function autostop.main(c) 
        ...
        if not on_ground or air_blocked then
            return
        end
...
local auto_os do
    ...
    local function auto_headshots_hotkey()
        ...
        local condition_map = {
            ...
            ["Crouch"] = menu.ragebot.cond:get("Crouch"),
            ["Air crouch"] = menu.ragebot.cond:get("Air crouch"),
            ["Air"] = menu.ragebot.cond:get("Air"),
...
local automatic_teleport do
    ...
    local function on_setup_command(cmd)
        ...
        if trigger then
            ...
            if menu.ragebot.auto_tp_air_check:get() then
                local aa_state = antiaim.state() 
                if aa_state ~= "Air" and aa_state ~= "Air crouch" then safe_to_tp = false end
            end
...
local force_shot_logic = {} do
    ...
    function force_shot_logic.is_regular_hitchance_priority_active(weapon_idx)
        ...
        local is_in_air = cur_state == "Air" or cur_state == "Air crouch"
    ...
    function force_shot_logic.handle_logic()
        ...
        utils.set_event_callback("paint", function()
            ...
            if force_hotkey_blocked and not hs_checker then
                renderer.indicator(255, 105, 105, 255, "FORCE SHOT")
            elseif force_active then
                renderer.indicator(130, 200, 255, 255, "FORCE SHOT")
            end
        end, state)
...
local crosshair = {} do
    ...
    function crosshair.paint()
        ...
        for _, elem in ipairs(crosshair_elements) do
            ...
            local display_text = (master_alpha < 0.99) and 'Archangel' or elem.text
...
local arrows = {} do
    ...
    function arrows.paint()
        ...
        for side, pos in pairs({left = arrows_positions.left, right = arrows_positions.right}) do
            ...
            if menu.visuals.arrows_type:get() == "Archangel" then
...
local feature_ind = {} do
    ...
    local function format_display_text(text)
        if menu.visuals.custom_ind_gs_icon:get() then
            ...
            elseif text == 'DA' then
                return ' DA'
            elseif text == 'FS' or text == 'FORCE SHOT' then
                return ' force shot'
            elseif text == 'MD' then
                return ' MD'
...
local solus = {} do
    ...
    function solus.watermark()
        ...
        if menu.visuals.water_main:get("Build") then
            table.insert(info_elements, (info.build:gsub("Archangel", "")))
        end
    ...
    local binds = {
        ...
        {nil, "Force second zoom", function()
            _, res = menu.visuals.zoom_hotkey:get()
            act = menu.visuals.zoom_hotkey:get() and menu.visuals.zoom:get()
            return act and true or false, res
        end},
...
local clantag = {} do 
    function clantag.anim(text, indices)
        local text_anim = "&,a%,ar@,arch|,archa#,archang&,Archangel6,Archangelho#,#ArchangelH66K,Archangel, Archangel,Archangel,#ArchangelH66K,Archangelhoo&,Archangelho6,Archangelh@,Archangel#,archang&,archa#,arch|,ar@,a%,&"
        local sequences = {}
    ...
    function clantag.clantag_handle()
        ...
        if misctab_clantag then
            ...
            if local_player ~= nil and globals.chokedcommands() == 0 then
                ...
                local text = clantag.anim("Archangel", {})
...
local phrases = {
    kill = {
        {'ARHANGEL 𝕋ℍ𝔼 𝔹𝔼𝕊𝕋 𝔾𝕄 𝔸𝔸', 0.3},
        {'again logged into discord and see', 0.3, 'best lua .->gg/uhVsNJGk', 1.0},
        {"𝕃𝕦𝕔𝕚𝕗𝕖𝕣 𝕖𝕞𝕡𝕚𝕣𝕖 𝕗𝕖𝕝𝕝 𝕗𝕣𝕠𝕞 Archangel", 0.3},
        {'Archangel 𝙥𝙧𝙚𝙙𝙞𝙘𝙩 𝙞𝙨 𝙩𝙝𝙚 𝙫𝙤𝙞𝙘𝙚 𝙤𝙛 𝙮𝙤𝙪𝙧 𝙙𝙚𝙖𝙩𝙝', 0.3},
        {"ℕ𝕆 𝕄𝔼ℝℂ𝕐", 0.3},
        {'1', 1.0},
        {'𝔾𝕠𝕕 𝕘𝕒𝕧𝕖 𝕞𝕖 𝕥𝕙𝕖 𝕤𝕥𝕣𝕖𝕟𝕘𝕥𝕙 𝕥𝕠 𝕜𝕚𝕝𝕝 𝕃𝕦𝕔𝕚𝕗𝕖𝕣', 0.3},
        {"𝑰 𝒅𝒐𝒏'𝒕 𝒄𝒐𝒎𝒑𝒆𝒕𝒆, 𝑰 𝒅𝒐𝒎𝒊𝒏𝒂𝒕𝒆 #Archangel", 0.3},
        {'1', 1.0},
        {'𝕪𝕠𝕦𝕣 𝕓𝕝𝕠𝕠𝕕 𝕚𝕤 𝕠𝕟 𝕞𝕪 𝕔𝕝𝕒𝕨𝕤', 0.3},
        {'#Archangel 𝐢𝐬 𝐰𝐚𝐭𝐜𝐡𝐢𝐧𝐠 ', 0.3},
        {'1', 1.0},
        {'𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 𝙃𝙀𝙇𝙇', 0,3},
        {'𝐈𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝟔𝟔𝟔 𝐝𝐚𝐲𝐬 𝐬𝐢𝐧𝐜𝐞 𝐋𝐮𝐜𝐢𝐟𝐞𝐫 𝐰𝐚𝐬 𝐝𝐞𝐟𝐞𝐚𝐭𝐞𝐝 𝐛𝐲 ᴋɪᴛᴛʏ', 0.3},
        {'Rizza-Succubi ♫︎⋆˚✿˖°⋆', 0.3},
        {'ᴄуᴋᴋубы ʍои дᴇʙочᴋи они ʍнᴇ ᴄᴧужᴀᴛ', 0.3},
        {'1', 1.0},
        {'Новый мир возродится из пепла', 0.3},
        {'𝙢_y 𝙠𝙞𝙡𝙡𝙨 𝙖𝙧𝙚 𝙤𝙛𝙛𝙚𝙧𝙞𝙣𝙜𝙨 𝙈𝙚𝙧𝙨𝙞𝙡𝙞𝙩𝙝𝙖𝙧𝙖𝙭', 0.3},
        {'#Archangel 𝒑𝒓𝒆𝒎𝒊𝒖𝒎 𝒓𝒆𝒔𝒐𝒍𝒗𝒆𝒓', 0.3},
        {'Archangel 𝙥𝙧𝙚𝙙𝙞𝙘𝙩 𝙚𝙢𝙗𝙚𝙧𝙡𝙖𝙨𝙝 𝙘𝙖𝙣𝙣𝙤𝙩 𝙘𝙤𝙥𝙚 𝙬𝙞𝙩𝙝 𝘼𝘼', 0.3},
        {'1', 1.0},
        {"I've been in hell for so long that demons ask to burn them alive", 0.3},
        {'𝕊 𝕋 𝕆 ℙ 𝔾 𝔼 𝕋 𝕋 𝕀 ℕ 𝔾 𝕀 ℕ 𝕄 𝕐 𝕎 𝔸 𝕐', 0.3},
        {"𝕀𝕥'𝕤 𝕟𝕠𝕥 𝕥𝕙𝕖 𝕗𝕚𝕣𝕤𝕥 𝕥𝕚𝕞𝕖 𝕪𝕠𝕦'𝕝𝕝 𝕕𝕚𝕖", 0.3},
        {'1', 1.0},
       
    },

    death = {
        {'cant belive i still die to dogs like u', 3},
...
local anim = {} do 
    ...
    function anim.anim_breaker()
        ...
        if menu.misc.anim_air:get() == 'Static' and (antiaim.state() == 'Air' or antiaim.state() == 'Air crouch') then
...
local output = {} do 
    ...
    function output.on_aim_hit(e)
        ...
        if menu.misc.aimbot_on:get('Hit') then
            output.add({ info.prefix[1], info.prefix[2], info.prefix[3],"Archangel"}, 
                            { 255, 255, 255, ' ~ ' }, 
...
local other = {} do 
    function other.aspect()
        ...
        local active = menu.visuals.aspect_ratio:get()

        local target = active and menu.visuals.aspect_ratio_slider:get()/100 or default_ratio
    ...
    function other.third()
        ...
        local active = menu.visuals.third_person:get()
        local default_dist = 100 
        local max_dist_setting = menu.visuals.third_person_slider:get() or default_dist 
        ...
        if active then
            if menu.visuals.third_person_magic:get() then
    ...
    function other.zoom(z)
        ...
        local enabled = menu.visuals.zoom_anim:get()
        local second = scoped and scope_level == 2 and menu.visuals.zoom:get() and menu.visuals.zoom_hotkey:get() and (enabled and -menu.visuals.zoom_fov:get() or -menu.visuals.zoom_fov:get()) or 0 
    ...
    function other.zoom_paint()
        if menu.visuals.zoom:get() and menu.visuals.zoom_hotkey:get() then
            renderer.indicator(230, 230, 230, 230, "ZOOM")
        end
    end
    ...
    function other.viewmodel()
        ...
        local active = menu.visuals.viewmodel:get()

        local t_fov = active and menu.visuals.vS:get() or def_fov
        local t_x   = active and menu.visuals.xS:get()/10 or def_x
        local t_y   = active and menu.visuals.yS:get()/10 or def_y
        local t_z   = active and menu.visuals.zS:get()/10 or def_z
    ...
    function other.hand()
        local is_enabled = menu.visuals.opposite_hand:get() and menu.visuals.viewmodel:get()
    ...
    function other.sleeves_update()
        local remove_sleeve = menu.visuals.remove_sleeves:get() and menu.visuals.viewmodel:get()
    ...
    menu.visuals.remove_sleeves:set_callback(function()
        if menu.visuals.remove_sleeves:get() then
            other.set_remove_sleeves(true)
        else
            other.reset_sleeves()
        end
...
local scp = {} do
    ...
    function scp.scope_overlay()
        local master_alpha = motion.new('scope_master_alpha', menu.visuals.scope_overlay:get() and 1 or 0, 0.1)

        if not menu.visuals.scope_overlay:get() and master_alpha < 0.01 and is_callback_active then
        ...
        local offset, initial_position, color, thickness =
            menu.visuals.scope_gap:get() * height / 1080,
            menu.visuals.scope_size:get() * height / 1080,
            { menu.visuals.scope_color:get() }, menu.visuals.scope_thickness:get()
        ...
        if menu.visuals.scope_overlay:get() then
            ...
            local rotate_progress = motion.new('scope_rotate_anim', menu.visuals.scope_position:get() == 'Rotate' and 1 or 0, 0.05)
            local alpha = motion.new('overlay_alpha', act and 1 or 0, menu.visuals.scope_animation:get('Alpha') and 0.05 or 2)
            local overlay = motion.new('scope_overlay', act and 1 or 0, menu.visuals.scope_animation:get('Overlay') and 0.05 or 2)
            local is_v2 = menu.visuals.scope_mode_ovr:get() == "V2"
            ...
            local function draw_universal_branch(base_angle, dir_name)
                if menu.visuals.scope_disablers:get(dir_name) then return end
    ...
    function scp.scpview()
        ...
        inscope = not menu.visuals.viewinsc:get()
        res.hide_vm_scope = inscope
    end
    menu.visuals.scope_overlay:set_callback(function()
        if menu.visuals.scope_overlay:get() then
            is_callback_active = true
...
local startup_animation = {} do
    local fade_duration = 1.0
    local display_duration = 2.0
    local slide_duration = 1.5
    local total_duration = fade_duration + display_duration + slide_duration
    local alpha = 0
    local bg_alpha = 0
    local logo_texture_id = nil
    local logo_loaded = false
    local screen_w, screen_h = client.screen_size()
    local anim_start_time = globals.realtime()
    _G.archangel_intro_done = false
    ...
    for i = 1, 80 do
        table.insert(particles, {
            ...
            speed_y = math.random(15, 45) * 0.1,
            speed_x = (math.random() - 0.5) * 25 * 0.1,
    ...
    local function draw_startup()
        if not logo_loaded then
            -- Use the original high-resolution transparent PNG for optimal scaling
            local file_content = readfile('csgo/archangel_logo.png')
            if not file_content then file_content = readfile('archangel_logo.png') end
            if file_content then
                logo_texture_id = renderer.load_png(file_content, 553, 685)
            end
            logo_loaded = true
        end

        local elapsed_time = globals.realtime() - anim_start_time

        if elapsed_time > total_duration then
            _G.archangel_intro_done = true
            client.unset_event_callback("paint_ui", draw_startup)
            return
        end

        local menu_x, menu_y = ui.menu_position()
        if not menu_x then menu_x, menu_y = screen_w / 2 - 300, screen_h / 2 - 200 end
        local menu_w, menu_h = ui.menu_size()
        if not menu_w then menu_w = 660 end

        -- Target configuration (final resting place above menu)
        local target_w, target_h = 300, 371
        local target_x = menu_x + (menu_w / 2) - (target_w / 2)
        local target_y = menu_y - target_h - 5

        -- Start configuration (center of screen, huge)
        local start_w, start_h = 553, 685
        local start_x = (screen_w / 2) - (start_w / 2)
        local start_y = (screen_h / 2) - (start_h / 2)

        local current_x, current_y, current_w, current_h = start_x, start_y, start_w, start_h
        ...
        if elapsed_time <= fade_duration then
            ...
            alpha = progress * 255
            bg_alpha = progress * 255
        elseif elapsed_time <= fade_duration + display_duration then
            -- Hold
            alpha = 255
            bg_alpha = 255
        else
            -- Slide, shrink, and fade out background
            alpha = 255
            local slide_time = elapsed_time - (fade_duration + display_duration)
            local progress = math.min(1.0, slide_time / slide_duration)
            local eased_progress = smoothstep(progress)

            bg_alpha = (1.0 - progress) * 255
            current_x = lerp(start_x, target_x, eased_progress)
            current_y = lerp(start_y, target_y, eased_progress)
            current_w = lerp(start_w, target_w, eased_progress)
            current_h = lerp(start_h, target_h, eased_progress)
        end

        alpha = math.max(0, math.min(255, alpha))
        bg_alpha = math.max(0, math.min(255, bg_alpha))

        -- Draw background
        renderer.rectangle(0, 0, screen_w, screen_h, 6, 6, 11, math.floor(bg_alpha * 0.95))

        -- Update and Draw Particles
        local frametime = globals.frametime()
        if frametime > 0 then
            for i, p in ipairs(particles) do
                p.y = p.y - (p.speed_y * frametime * 50)
                p.x = p.x + (p.speed_x * frametime * 50)
                
                -- Boundary loop
                if p.y < -10 then p.y = screen_h + 10 end
                if p.x < -10 then p.x = screen_w + 10 end
                if p.x > screen_w + 10 then p.x = -10 end

                -- Draw particle (Soft gold/white to match Archangel logo)
                renderer.circle(math.floor(p.x), math.floor(p.y), 255, 235, 180, math.floor(bg_alpha * 0.6), p.size, 0, 1)
            end
        end

        -- Draw logo
        if logo_texture_id then
            -- We introduce the 'f' flag to allow dynamic scaling during the slide
            renderer.texture(logo_texture_id, math.floor(current_x), math.floor(current_y), math.floor(current_w), math.floor(current_h), 255, 255, 255, math.floor(alpha), 'f')
        end
    end

    client.set_event_callback("paint_ui", draw_startup)
end

local button_check = false

local cfg_system = {} do
    local file_path = "Archangel.json"
    ...
    menu.information.button_export:set_callback(function()
        ...
        clipboard.set("Archangel_" .. encoded_str)
    ...
    menu.information.button_import:set_callback(function()
        ...
        if not input or not input:find("Archangel_") then
            client.log("Error: Invalid clipboard data")
            return
        end
        local cleaned = input:gsub("Archangel_", "")
...
local pizdec do   
    ...
    local function ui2()
        ...
        local color_func = coloring.color(
            ...
            menu.information.user:set(color_func("User: ") .. "\r" .. info.name)
            menu.information.build:set(color_func("Build:") .. "\r" .. info.build:gsub("Archangel", ""))
            ...
            lines21:set(color_func("— [ Configurations Settings ] —"))

            menu.ragebot.lb_forceshot:set(color_func('— [ Force Shot ] —'))
            menu.antiaim.label:set(color_func("— [ Anti-aim condition ] —"))
            ...
            for i = 1, #antiaim_cond do
                ...
                builder[i].label2:set(color_func("— [ Yaw Settings ] —"))
                builder[i].label5:set(color_func("— [ Yaw Modifiers ] —"))
                builder[i].label6:set("\rJitter " .. color_func("Mode"))
                builder[i].label9:set(color_func("Body ") .. "\r" .. "Yaw")
                builder[i].label11:set(color_func("— [ Other Settings ] —"))
                builder[i].lb_brute:set(color_func("— [ Antibrute Settings ] —"))
                builder[i].label12:set(color_func("— [ Defensive Settings ] —"))
...
local function setup_archangel_logo()
    local logo_texture_id = nil
    local target_width = 300
    local target_height = 371
    local logo_loaded = false

    local function load_menu_logo()
        if logo_loaded then return end
        
        -- Load the pre-scaled 300x371 PNG
        local file_content = readfile('csgo/archangel_logo_scaled.png')
        if not file_content then
            file_content = readfile('archangel_logo_scaled.png')
        end
        
        if file_content then
            -- Gamesense load_png MUST have 3 arguments or it throws an error!
            local texture = renderer.load_png(file_content, target_width, target_height)
            if texture ~= nil then
                logo_texture_id = texture
                logo_loaded = true
            else
                client.color_log(255, 0, 0, "Archangel: load_png returned nil")
                logo_loaded = true -- prevent spam
            end
        else
            client.color_log(255, 0, 0, "Archangel: Could not find archangel_logo_scaled.png")
            logo_loaded = true -- prevent spam
        end
    end
    ...
    client.set_event_callback("paint_ui", function()
        if not ui.is_menu_open() then return end
        if not _G.archangel_intro_done then return end
        
        load_menu_logo()
        if not logo_texture_id then return end
        
        local menu_x, menu_y = ui.menu_position()
        local menu_w, menu_h = ui.menu_size()
        if menu_x == nil or menu_w == nil then return end
        
        local draw_x = menu_x + (menu_w / 2) - (target_width / 2)
        local draw_y = menu_y - target_height - 5
        
        -- Draw without the 'f' flag to allow native alpha blending
        renderer.texture(logo_texture_id, draw_x, draw_y, target_width, target_height, 255, 255, 255, 255)
    end)
end

setup_archangel_logo()
